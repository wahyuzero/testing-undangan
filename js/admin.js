...
// ========================================
const ADMIN_CONFIG = {
    // JSONBin untuk RSVP/Messages
    jsonbin: {
        binId: '693e214b43b1c97be9ecb25e', // Bin ID untuk RSVP
        accessKey: '$2a$10$vhb4wS1kT46UDn1caaOt0eukxI/Or4f7wPEBub813DsfmDImNQMuK'
    },

    // JSONBin untuk Daftar Tamu (Buat bin baru untuk ini)
    jsonbinGuests: {
        binId: '693c77f5d0ea881f4025105f', // Ganti dengan Bin ID baru (struktur: {"invitedGuests":[],"specialGuests":[]})
        accessKey: '$2a$10$vhb4wS1kT46UDn1caaOt0eukxI/Or4f7wPEBub813DsfmDImNQMuK'
    },
    ...
    // Base URL untuk invitation links
    invitationBaseUrl: 'https://kukuh-fitriani.site/',
...
// ========================================
// JSONBIN API (Standalone)
// ========================================
const AdminJSONBinAPI = {
    baseUrl: 'https://api.jsonbin.io/v3',

    getHeaders() {
        return {
            'Content-Type': 'application/json',
            'X-Access-Key': ADMIN_CONFIG.jsonbin.accessKey
        };
    },

    async getData() {
        try {
            const response = await fetch(`${this.baseUrl}/b/${ADMIN_CONFIG.jsonbin.binId}/latest`, {
                method: 'GET',
    ...
    async saveData(data) {
        try {
            const response = await fetch(`${this.baseUrl}/b/${ADMIN_CONFIG.jsonbin.binId}`, {
                method: 'PUT',
                headers: this.getHeaders(),
    ...
    async getGuestsData() {
        if (ADMIN_CONFIG.jsonbinGuests.binId === 'YOUR_GUESTS_BIN_ID') {
            return { invitedGuests: [], specialGuests: [] };
        }

        try {
            const response = await fetch(`${this.baseUrl}/b/${ADMIN_CONFIG.jsonbinGuests.binId}/latest`, {
                method: 'GET',
                headers: {
                    'X-Access-Key': ADMIN_CONFIG.jsonbinGuests.accessKey
                }
            });

            if (response.ok) {
                const data = await response.json();
                return data.record || { invitedGuests: [], specialGuests: [] };
            }
            return { invitedGuests: [], specialGuests: [] };
        } catch (error) {
    ...
    async saveGuestsData(data) {
        if (ADMIN_CONFIG.jsonbinGuests.binId === 'YOUR_GUESTS_BIN_ID') {
            console.warn('Guests JSONBin not configured');
            return false;
        }

        try {
            const response = await fetch(`${this.baseUrl}/b/${ADMIN_CONFIG.jsonbinGuests.binId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Access-Key': ADMIN_CONFIG.jsonbinGuests.accessKey
...
class AdminPanel {
    constructor() {
        ...
        this.defaultPassword = '@KukuhFitri990011';
    ...
    bindEvents() {
        ...
        // Filter
        document.getElementById('filterAttendance')?.addEventListener('change', (e) => this.handleFilter(e));
    ...
    handleLogin(e) {
        ...
        const storedPassword = localStorage.getItem('adminPassword') || this.defaultPassword;
    ...
    async loadData() {
        try {
            ...
            this.data = await AdminJSONBinAPI.getData();
    ...
    async executeDelete() {
        ...
        try {
            ...
            this.data.guests = (this.data.guests || []).filter(g => g.id !== this.deleteTargetId);

            // Update JSONBin
            const response = await fetch(`${AdminJSONBinAPI.baseUrl}/b/${ADMIN_CONFIG.jsonbin.binId}`, {
                method: 'PUT',
                headers: AdminJSONBinAPI.getHeaders(),
    ...
    updateSettingsInfo() {
        ...
        document.getElementById('binIdDisplay').textContent = ADMIN_CONFIG.jsonbin.binId.substring(0, 12) + '...';
    }
    ...
    async loadGuestsData() {
        try {
            // Check if guests bin is configured
            if (ADMIN_CONFIG.jsonbinGuests.binId === 'YOUR_GUESTS_BIN_ID') {
                console.warn('⚠️ Guest JSONBin not configured');
                this.guestsData = { invitedGuests: [], specialGuests: [] };
            ...
            this.guestsData = await AdminJSONBinAPI.getGuestsData();
            this.renderGuestsTable();
            this.renderSpecialGuests();
        } catch (error) {
    ...
    async saveGuestsData() {
        if (ADMIN_CONFIG.jsonbinGuests.binId === 'YOUR_GUESTS_BIN_ID') {
            this.showToast('JSONBin untuk tamu belum dikonfigurasi', 'error');
            return false;
        }

        return await AdminJSONBinAPI.saveGuestsData(this.guestsData);
    }
    ...
    async handleAddGuest(e) {
        ...
        const saved = await this.saveGuestsData();
        if (saved) {
    ...
    generateInvitationLink(name) {
        const baseUrl = ADMIN_CONFIG.invitationBaseUrl || (window.location.origin + window.location.pathname.replace('admin.html', ''));
